...
run-restart: ## restart the API server
	...
	@go run ${LDFLAGS} main.go run & echo $$! > $(PID_FILE)
	@printf '%*s\n' "80" '' | tr ' ' -
...
build:  ## build the API server binary
	CGO_ENABLED=0 go build ${LDFLAGS} -a -o comvoca $(MODULE)

.PHONY: build-docker
build-docker: ## build the API server as a docker image
	docker build -f Dockerfile -t comvoca:$(VERSION) .
...
db-start: ## start the database server
	...
	docker run --rm --name postgres \
		-e POSTGRES_PASSWORD=postgres -e POSTGRES_DB=comvoca -d -p 5432:5432 postgres
